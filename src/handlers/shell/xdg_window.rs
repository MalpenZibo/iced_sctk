...
impl<T: Debug> WindowHandler for SctkState<T> {
    ...
    fn configure(
        ...
        if window.last_configure.as_ref().map(|c| c.state) != Some(configure.state) {
            self.sctk_events.push(SctkEvent::WindowEvent {
                variant: WindowEventVariant::StateChanged(configure.state),
                id: window.window.wl_surface().clone(),
            });
        }
        if window.last_configure.as_ref().map(|c| c.capabilities) != Some(configure.capabilities) {
            self.sctk_events.push(SctkEvent::WindowEvent {
                variant: WindowEventVariant::WmCapabilities(configure.capabilities),
                id: window.window.wl_surface().clone(),
            });
        }
