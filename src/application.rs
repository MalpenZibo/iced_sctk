...
use crate::{
    clipboard::Clipboard,
    command::wayland::{self, data_device::DndIcon, popup},
    ...
    settings,
    widget::operation::{self, MapOperation, OperationOutputWrapper, OperationWrapper},
};
...
use iced_core::{
    widget::{operation::Outcome, Operation},
...
use iced_runtime::{
    ...
    system, user_interface, Command, Debug, UserInterface,
};
...
use std::{
    collections::HashMap, hash::Hash, marker::PhantomData, os::raw::c_void, ptr::NonNull,
    time::Duration,
};
...
pub enum Event<Message> {
    ...
    LayerSurface(wayland::layer_surface::Action<Message>),
    /// window requests from the client
    Window(wayland::window::Action<Message>),
    /// popup requests from the client
    Popup(wayland::popup::Action<Message>),
    /// data device requests from the client
    DataDevice(wayland::data_device::Action<Message>),
    /// xdg-activation request from the client
    Activation(wayland::activation::Action<Message>),
    /// data session lock requests from the client
    SessionLock(wayland::session_lock::Action<Message>),
...
impl HasDisplayHandle for SurfaceDisplayWrapper {
    fn display_handle(&self) -> Result<DisplayHandle, HandleError> {
        ...
        Ok(unsafe { DisplayHandle::borrow_raw(RawDisplayHandle::Wayland(display_handle)) })
    }
}
...
impl HasWindowHandle for SurfaceDisplayWrapper {
    fn window_handle(&self) -> Result<WindowHandle, HandleError> {
        ...
        Ok(unsafe { WindowHandle::borrow_raw(RawWindowHandle::Wayland(window_handle)) })
    }
}
    ...
    <A as Program>::Theme: StyleSheet,
{
    ...
    let _ = event_loop.run_return(move |event, _, control_flow| {
        ...
        match poll {
            ...
            task::Poll::Ready(_) => *control_flow = ControlFlow::ExitWithCode(1),
        };
    });
    ...
    <A as Program>::Theme: StyleSheet,
{
    ...
    #[cfg(feature = "a11y")]
    let mut a11y_events: Vec<crate::sctk_event::ActionRequestEvent> = Vec::new();
    #[cfg(feature = "a11y")]
    let mut a11y_enabled = false;
    #[cfg(feature = "a11y")]
    let mut adapters: HashMap<SurfaceId, crate::event_loop::adapter::IcedSctkAdapter> =
        HashMap::new();
    ...
    'main: while let Some(event) = receiver.next().await {
        match event {
                ...
                match event {
                    ...
                    SctkEvent::PointerEvent { variant, .. } => {
                            ...
                            .and_then(|id| states.get_mut(&id.inner()).map(|state| (state, id)))
                        {
                            Some(s) => s,
                            None => {
                                if let Some((x_offset, y_offset, id)) =
                        ...
                        match variant.kind {
                            PointerEventKind::Enter { .. } => {
                                state.set_cursor_position(Some(LogicalPosition {
                                    x: variant.position.0 + offset.0,
                                    y: variant.position.1 + offset.1,
                                }));
                            }
                    ...
                    SctkEvent::TouchEvent {
                        ...
                        state.set_cursor_position(Some(LogicalPosition {
                            x: position.x as f64 + offset.0,
                            y: position.y as f64 + offset.1,
                        }));
                    }
                    SctkEvent::WindowEvent {
                        ...
                        crate::sctk_event::WindowEventVariant::Created(id, native_id) => {
                            ...
                            states.insert(
                                ...
                                State::new(
                                    &application,
                                    SurfaceIdWrapper::Window(native_id),
                        ...
                        crate::sctk_event::WindowEventVariant::Configure(
                            ...
                            if let Some(id) = surface_ids.get(&wl_surface.id()) {
                                ...
                                if state.surface.is_none() {
                                    ...
                                    if matches!(
                                        simple_clipboard.state(),
                                        crate::clipboard::State::Unavailable
                                    ) {
                                        if let Ok(h) = wrapper.display_handle() {
                                            simple_clipboard = unsafe { Clipboard::connect(&h) };
                                        }
                                    }
                                    let mut c_surface = compositor.create_surface(
                                        wrapper.clone(),
                                ...
                                if let Some((w, h, _, is_dirty)) = auto_size_surfaces.get_mut(id) {
                                    *is_dirty = first
                                    ...
                                    state.set_logical_size(
                                        configure.new_size.0.unwrap().get() as f32,
                                        configure.new_size.1.unwrap().get() as f32,
                                    );
                                }
                                if first {
                    ...
                    SctkEvent::LayerSurfaceEvent {
                        ...
                        LayerSurfaceEventVariant::Configure(configure, wl_surface, first) => {
                            if let Some(id) = surface_ids.get(&wl_surface.id()) {
                                ...
                                if let Some((w, h, _, is_dirty)) = auto_size_surfaces.get_mut(id) {
                                    *is_dirty = first
                                        || *w != configure.new_size.0
                                        || *h != configure.new_size.1;
                                    state.set_logical_size(*w as f32, *h as f32);
                                } else {
                                    state.set_logical_size(
                    ...
                    SctkEvent::PopupEvent {
                        ...
                        PopupEventVariant::Created(id, native_id) => {
                            ...
                            states.insert(
                                ...
                                State::new(
                                    &application,
                                    SurfaceIdWrapper::Popup(native_id),
                                    SurfaceDisplayWrapper {
                                        backend: backend.clone(),
                                        wl_surface,
                                    },
                        ...
                        PopupEventVariant::Configure(configure, wl_surface, first) => {
                            if let Some(id) = surface_ids.get(&wl_surface.id()) {
                                ...
                                if let Some((w, h, _, is_dirty)) = auto_size_surfaces.get_mut(id) {
                                    *is_dirty |= first
                                        || *w != configure.width as u32
                                        || *h != configure.height as u32;
                                    state.set_logical_size(*w as f32, *h as f32);
                                } else {
                                    state.set_logical_size(
                        ...
                        PopupEventVariant::Size(width, height) => {
                            if let Some(id) = surface_ids.get(&wl_surface.id()) {
                                if let Some(state) = states.get_mut(&id.inner()) {
                                    if let Some((w, h, _, is_dirty)) =
                                        auto_size_surfaces.get_mut(id)
                                    {
                                        *is_dirty = *w != width || *h != height;
                                        state.set_logical_size(*w as f32, *h as f32);
                                    } else {
                                        state.set_logical_size(width as f32, height as f32);
                                    }
                                }
                            }
                        }
                    ...
                    SctkEvent::DataSource(DataSourceEvent::DndFinished)
                    | SctkEvent::DataSource(DataSourceEvent::DndCancelled) => {
                    ...
                    SctkEvent::SessionLockSurfaceCreated { surface, native_id } => {
                        ...
                        states.insert(
                            ...
                            State::new(
                                &application,
                                SurfaceIdWrapper::SessionLock(native_id),
                                SurfaceDisplayWrapper {
                                    backend: backend.clone(),
            ...
            IcedSctkEvent::DndSurfaceCreated(wl_surface, dnd_icon, origin_id) => {
                ...
                let mut state = State::new(&application, SurfaceIdWrapper::Dnd(native_id), wrapper);
                state.surface = Some(c_surface);
                state.set_logical_size(w as f32, h as f32);
            ...
            IcedSctkEvent::MainEventsCleared => {
                if !redraw_pending && sctk_events.is_empty() && messages.is_empty() {
                    continue;
                }
                ...
                if surface_ids.is_empty() && !messages.is_empty() {
                    ...
                    for (object_id, surface_id) in &surface_ids {
                        ...
                        while i < sctk_events.len() {
                            let has_kbd_focus = kbd_surface_id.as_ref() == Some(object_id);
                        ...
                        #[cfg(feature = "a11y")]
                        {
                            let mut filtered_a11y = Vec::with_capacity(a11y_events.len());
                            ...
                            native_events.extend(filtered_a11y.into_iter().map(|e| {
                                iced_runtime::core::event::Event::A11y(
                                    iced_runtime::core::id::Id::from(
                                        u128::from(e.request.target.0) as u64,
                                    ),
                                    e.request,
                                )
                            }));
                        }
                        let has_events = has_events || !native_events.is_empty();

                        let (interface_state, statuses) = {
                            let Some(user_interface) = interfaces.get_mut(&surface_id.inner())
                            else {
                                continue;
                            };
                        ...
                        for (event, status) in native_events.into_iter().zip(statuses.into_iter()) {
                        ...
                        if redraw_pending || needs_update {
                            state.set_needs_redraw(state.frame_pending || needs_update);
                    ...
                    if needs_update {
                        ...
                        for surface_id in surface_ids.values() {
                            ...
                            let mut cache = match pure_states.remove(&surface_id.inner()) {
                                Some(cache) => cache,
                    ...
                    for (object_id, surface_id) in &surface_ids {
                                ...
                                if !s.needs_redraw()
                                    || auto_size_surfaces
                                        ...
                                        .map(|(w, h, _, dirty)| {
                                            ...
                                            *dirty || {
                                                let Size { width, height } = s.logical_size();
                                                width.ceil() as u32 != *w
                                                    || height.ceil() as u32 != *h
                                            }
                                        })
                                        .unwrap_or_default()
                                {
                        ...
                        let redraw_event = CoreEvent::Window(
                            surface_id.inner(),
                            crate::core::window::Event::RedrawRequested(Instant::now()),
                        );
                        let Some(user_interface) = interfaces.get_mut(&surface_id.inner()) else {
                            continue;
                        };
                        ...
                        let _ = control_sender.start_send(match interface_state {
                            user_interface::State::Updated {
                                ...
                                crate::core::window::RedrawRequest::NextFrame => ControlFlow::Poll,
                    ...
                    if !sent_control_flow {
                        if let Some(d) = wait {
                            ...
                            wait_until += d;
                            _ = control_sender.start_send(ControlFlow::WaitUntil(wait_until));
                        } else {
            ...
            IcedSctkEvent::RedrawRequested(object_id) => {
                if let Some((native_id, Some(mut user_interface), Some(state))) =
                    ...
                    })
                {
                    ...
                    #[cfg(feature = "a11y")]
                    if let Some(Some(adapter)) =
                        a11y_enabled.then(|| adapters.get_mut(&native_id.inner()))
                    {
                        use iced_accessibility::{
                        ...
                        let mut current_operation = Some(Box::new(OperationWrapper::Id(Box::new(
                    ...
                    if state.frame_pending {
                        ...
                        state
                            .wrapper
                            .wl_surface
                            .frame(&queue_handle, state.wrapper.wl_surface.clone());
                    }
                    let _ = compositor.present(
                        &mut renderer,
                        &mut comp_surface,
            ...
            IcedSctkEvent::A11yEvent(ActionRequestEvent {
                ...
                match request.action {
                    ...
                    Action::Focus => {
                        commands.push(Command::widget(operation::focusable::focus(
                            iced_runtime::core::id::Id::from(u128::from(request.target.0) as u64),
                        )));
                    }
                    Action::Blur => todo!(),
    ...
    <A as Program>::Theme: StyleSheet,
{
    ...
    let size =
        if let Some((auto_size_w, auto_size_h, limits, dirty)) = auto_size_surfaces.remove(&id) {
                ...
                match id {
                    SurfaceIdWrapper::LayerSurface(inner) => {
                        ev_proxy.send_event(Event::LayerSurface(
                            crate::command::wayland::layer_surface::Action::Size {
                                id: inner,
                                width: Some(w),
                                height: Some(h),
                            },
                    ...
                    SurfaceIdWrapper::Window(inner) => {
                        ev_proxy.send_event(Event::Window(
                            crate::command::wayland::window::Action::Size {
                                id: inner,
                                width: w,
                                height: h,
                            },
                    ...
                    SurfaceIdWrapper::Popup(inner) => {
                        ev_proxy.send_event(Event::Popup(
                            crate::command::wayland::popup::Action::Size {
                                id: inner,
                                width: w,
                                height: h,
                            },
    ...
    <A as Program>::Theme: application::StyleSheet,
{
    ...
    pub fn new(application: &A, id: SurfaceIdWrapper, wrapper: SurfaceDisplayWrapper) -> Self {
    ...
    pub fn set_logical_size(&mut self, w: f32, h: f32) {
            ...
            || !approx_eq!(f32, h as f32, old_size.height, F32Margin::default())
        {
            ...
            let physical_size = Size::new(
                (logical_size.width as f64 * self.scale_factor()).ceil() as u32,
                (logical_size.height as f64 * self.scale_factor()).ceil() as u32,
            );
            self.viewport = Viewport::with_physical_size(physical_size, self.scale_factor());
            // self.viewport = Viewport::with_logical_size(logical_size, self.scale_factor());
    ...
    pub fn set_scale_factor(&mut self, scale_factor: f64) {
        if !approx_eq!(
            ...
            let scale = self.application_scale_factor * self.surface_scale_factor;
            let physical_size = Size::new(
                (logical_size.width as f64 * scale).ceil() as u32,
                (logical_size.height as f64 * scale).ceil() as u32,
            );
            self.viewport = Viewport::with_physical_size(physical_size, scale);
    ...
    pub fn cursor(&self) -> mouse::Cursor {
        self.cursor_position
            .map(|cursor_position| {
                ...
                assert!(scale_factor.is_sign_positive() && scale_factor.is_normal());
                let logical: LogicalPosition<f64> = cursor_position.to_logical(scale_factor);
    ...
    pub fn set_cursor_position(&mut self, p: Option<LogicalPosition<f64>>) {
        self.cursor_position = p.map(|p| p.to_physical(self.application_scale_factor));
    }
    ...
    C: Compositor,
{
    match action {
        ...
        command::Action::Clipboard(action) => match action {
            ...
            clipboard::Action::Write(contents, kind) => clipboard.write(kind, contents),
        },
            ...
            while let Some(mut operation) = current_operation.take() {
                ...
                match operation.as_ref().finish() {
                    ...
                    Outcome::Some(message) => {
                        match message {
                            OperationOutputWrapper::Message(m) => {
                                proxy.send_event(Event::SctkEvent(IcedSctkEvent::UserEvent(m)));
                                ret = Some(operation)
                            }
        ...
        // TODO qui dovrei prendere la custom action e gestirla
        command::Action::Custom(custom) => {
            if let Ok(action) = custom.downcast::<wayland::Action<A::Message>>() {
                match *action {
                    wayland::Action::LayerSurface(layer_surface_action) => {
                        if let wayland::layer_surface::Action::LayerSurface {
                            mut builder,
                            _phantom,
                        } = layer_surface_action
                        {
                            ...
                            proxy.send_event(Event::LayerSurface(
                                wayland::layer_surface::Action::LayerSurface { builder, _phantom },
                            ));
                        } else {
                            proxy.send_event(Event::LayerSurface(layer_surface_action));
                        }
                    }
                    wayland::Action::Window(window_action) => {
                        if let wayland::window::Action::Window {
                            mut builder,
                            _phantom,
                        } = window_action
                        {
                            ...
                            proxy.send_event(Event::Window(wayland::window::Action::Window {
                                builder,
                                _phantom,
                            }));
                        } else {
                    ...
                    wayland::Action::Popup(popup_action) => {
                        ...
                        } = popup_action
                        {
                            ...
                            proxy.send_event(Event::Popup(popup::Action::Popup {
                                popup,
                    ...
                    wayland::Action::DataDevice(data_device_action) => {
                        proxy.send_event(Event::DataDevice(data_device_action));
                    }
                    wayland::Action::Activation(activation_action) => {
                        proxy.send_event(Event::Activation(activation_action));
                    }
                    wayland::Action::SessionLock(session_lock_action) => {
...
pub fn build_user_interfaces<'a, A, C>(
    ...
    SurfaceId,
    UserInterface<'a, <A as Program>::Message, <A as Program>::Theme, <A as Program>::Renderer>,
>
where
    A: Application + 'static,
    <A as Program>::Theme: StyleSheet,
