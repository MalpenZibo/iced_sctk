    ...
    T: 'static + Debug,
{
            ...
            if let Some(s) = self.seats.first() {
                popup.xdg_popup().grab(
                    ...
                    s.last_ptr_press.map(|p| p.2).unwrap_or_else(|| {
                        s.last_kbd_press.as_ref().map(|p| p.1).unwrap_or_default()
                    }),
                )
            }
        }
    ...
    pub fn get_window(&mut self, settings: SctkWindowSettings) -> (window::Id, WlSurface) {
        ...
        let window =
            self.xdg_shell_state
                .create_window(wl_surface.clone(), decorations, &self.queue_handle);
        ...
        let wp_viewport = self
            .viewporter_state
            .as_ref()
            .map(|state| state.get_viewport(window.wl_surface(), &self.queue_handle));
        let wp_fractional_scale = self
            .fractional_scaling_manager
            .as_ref()
        ...
        self.windows.push(SctkWindow {
            ...
            current_size: Some((NonZeroU32::new(1).unwrap(), NonZeroU32::new(1).unwrap())),
            last_configure: None,
    ...
    pub fn get_lock_surface(&mut self, id: window::Id, output: &WlOutput) -> Option<WlSurface> {
